...
async function fetchTripsInternal() {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("trips")
      .select("id, name, slug, category, duration, price, hero_image, short_description, tags, is_active, is_featured, created_at")
      .eq("is_active", true)
...
async function fetchTripNamesInternal() {
  if (!isSupabaseConfigured()) {
    return Object.values(localTripDetails).map(t => ({ id: t.id, name: t.name }));
  }

  try {
    const supabase = await getSupabaseClient();
    if (!supabase) return Object.values(localTripDetails).map(t => ({ id: t.id, name: t.name }));
    ...
    console.error("Error fetching trip names from Supabase:", err);
    return Object.values(localTripDetails).map(t => ({ id: t.id, name: t.name }));
  }
}

export const getTripNames = unstable_cache(
  fetchTripNamesInternal,
  ["trip-names-list"],
  { revalidate: CACHE_REVALIDATE, tags: ["trips"] }
);
...
async function fetchTripBySlugInternal(slug) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("trips")
      .select("id, name, slug, category, duration, price, hero_image, short_description, description, highlights, itinerary, gallery, inclusions, exclusions, tags, is_active, is_featured, created_at, updated_at")
      .eq("slug", slug)
      .single();
...
async function fetchFeaturedTripsByCategoryInternal(category) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("trips")
      .select("id, name, slug, category, duration, price, hero_image, short_description, tags, is_active, is_featured, updated_at")
      .eq("category", category)
      .eq("is_active", true)
      .eq("is_featured", true)
...
async function fetchReviewsInternal() {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("reviews")
      .select("id, name, designation, trip, quote, image, is_approved, is_featured, created_at")
      .eq("is_approved", true)
      .order("created_at", { ascending: false });
...
export async function getFeaturedReviews() {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("reviews")
      .select("id, name, designation, trip, quote, image, is_approved, is_featured")
      .eq("is_featured", true)
      .eq("is_approved", true)
      .limit(6);
...
async function fetchGalleryInternal() {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("gallery")
      .select("id, image_url, trip_id, display_order, alt_text")
...
async function fetchGroupTripsInternal() {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("group_trips")
      .select("id, name, slug, departure_date, hosted_place, capacity, price, hero_image, short_description, is_active")
      .eq("is_active", true)
...
export const getGroupTrips = unstable_cache(
  fetchGroupTripsInternal,
  ["group-trips-list"],
  { revalidate: CACHE_REVALIDATE, tags: ["group-trips"] }
);

async function fetchGroupTripBySlugInternal(slug) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("group_trips")
      .select("id, name, slug, departure_date, hosted_place, capacity, price, hero_image, short_description, description, highlights, itinerary, gallery, inclusions, exclusions, tags, is_active, is_featured, created_at, updated_at")
      .eq("slug", slug)
      .eq("is_active", true)
      .maybeSingle();
...
export async function getGroupTripBySlug(slug) {
  const cachedFn = unstable_cache(
    () => fetchGroupTripBySlugInternal(slug),
    [`group-trip-${slug}`],
    { revalidate: CACHE_REVALIDATE, tags: ["group-trips", `group-trip-${slug}`] }
  );
  return cachedFn();
}

export async function getGroupTripSlugs() {
  if (!isSupabaseConfigured()) return [];
  ...
  try {
    ...
    console.error("Error fetching group trip slugs:", err);
    return [];
  }
}
