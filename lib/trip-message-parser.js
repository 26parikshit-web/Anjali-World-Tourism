...
/**
 * Split a bulk upload file into individual package blocks.
 * Delimiter: ======== ... PACKAGE N ... ========
 */
export function splitTripPackages(messageText) {
  const chunks = messageText
    .split(/={10,}\s*\nPACKAGE\s+\d+\s*\n={10,}/i)
    .map((c) => c.trim())
    .filter((c) => c.includes("=== TRIP DETAILS ==="));

  if (chunks.length > 0) return chunks;

  if (messageText.includes("=== TRIP DETAILS ===")) {
...
/**
 * Parse one or many structured trip packages from a single file.
 */
function replacePaxWithPerson(value) {
  if (typeof value !== "string" || !value) return value;
  return value.replace(/\bPax\b/gi, "Person");
}

function normalizeTripTerminology(trip) {
  ...
  for (const key of [
    ...
    if (typeof normalized[key] === "string") {
      normalized[key] = replacePaxWithPerson(normalized[key]);
    }
  }
  for (const key of ["highlights", "inclusions", "exclusions", "tags"]) {
    if (Array.isArray(normalized[key])) {
      normalized[key] = normalized[key].map((item) =>
        replacePaxWithPerson(item)
      );
    }
  }
  if (Array.isArray(normalized.itinerary)) {
    normalized.itinerary = normalized.itinerary.map((day) => ({
      ...day,
      title: replacePaxWithPerson(day.title),
      description: replacePaxWithPerson(day.description),
    }));
  }
  return normalized;
}

export function parseTripMessages(messageText) {
  const chunks = splitTripPackages(messageText);
  return chunks.map((chunk) =>
    normalizeTripTerminology(parseStructuredFormat(chunk))
  );
}
...
function parseStructuredFormat(messageText) {
  ...
  if (itineraryMatch) {
    ...
    while ((dayMatch = dayRegex.exec(itinerarySection)) !== null) {
      ...
      const endDay = dayMatch[2] ? parseInt(dayMatch[2], 10) : null;
      const dayTitle = dayMatch[3].trim();
      const dayDescription = dayMatch[4].trim();
      const title =
        endDay && endDay !== startDay
          ? `Day ${startDay}–${endDay}: ${dayTitle}`
          : dayTitle;
  ...
  // Append preferences to description (no separate DB column)
  const preferencesMatch = messageText.match(
    /===\s*PREFERENCES\s*===\s*([\s\S]*?)(?=\n===|$)/i
  );
  if (preferencesMatch) {
    const section = preferencesMatch[1].trim();
    if (section) {
      result.description += "\n\n### Preferences\n" + section;
    }
  }
  ...
  if (tripInfoMatch) {
    ...
    const priceLines = section
      .split("\n")
      .map((l) => l.trim())
      .filter((l) =>
        /^(Price|Total Cost|Standard Price|Luxury Price)/i.test(l)
      );
    if (priceLines.length > 0) {
      result.price = priceLines.join(" | ");
    } else {
  ...
  // Extract Hotels (append to description)
  const hotelsMatch = messageText.match(
    /===\s*HOTELS\s*===\s*([\s\S]*?)(?=\n===|$)/i
  );
  if (hotelsMatch) {
    const hotelsInfo = hotelsMatch[1].trim();
    if (hotelsInfo) {
      result.description += "\n\n### Hotels\n" + hotelsInfo;
    }
  }
  ...
  return normalizeTripTerminology(result);
}
...
function normalizeCategory(raw) {
  ...
  if (lower.includes("international") || lower.includes("sri lanka")) {
    return "International";
  }
  if (lower.includes("family") || lower.includes("leisure")) {
    return "Family Time";
  }
  if (
    lower.includes("adventure") ||
    lower.includes("trek") ||
    lower.includes("hill station") ||
