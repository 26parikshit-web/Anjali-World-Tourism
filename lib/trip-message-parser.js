...
function parseStructuredFormat(messageText) {
  ...
  if (highlightsMatch) {
    result.highlights = extractBulletPoints(highlightsMatch[1]);
  }

  // Extract What's Included
  const includedMatch = messageText.match(
    /===\s*WHAT'S INCLUDED\s*===\s*([\s\S]*?)(?:===|$)/i
  );
  if (includedMatch) {
    result.inclusions = extractBulletPoints(includedMatch[1]);
  }

  // Extract What's Not Included
  const excludedMatch = messageText.match(
    /===\s*WHAT'S NOT INCLUDED\s*===\s*([\s\S]*?)(?:===|$)/i
  );
  if (excludedMatch) {
    result.exclusions = extractBulletPoints(excludedMatch[1]);
  }
...
function extractBulletPoints(content) {
  ...
  for (const line of lines) {
    ...
    trimmed = trimmed.replace(/​/g, ""); // Remove zero-width spaces
