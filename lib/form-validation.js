...
export function validateReviewPayload(data) {
  ...
  const mediaResult = validateReviewMedia(data);
  if (!mediaResult.valid) return mediaResult;

  return validationOk();
}

function validateReviewMedia(data) {
  const url = String(data?.image_url ?? "").trim();
  if (!url) return validationOk();

  if (url.length > 2048) {
    return validationError("Media URL is too long.");
  }

  if (!/^https:\/\//.test(url)) {
    return validationError("Media URL must be a secure HTTPS link.");
  }

  const resourceType = data?.resource_type;
  if (resourceType && resourceType !== "image" && resourceType !== "video") {
    return validationError("Invalid media type.");
  }
...
export function sanitizeReviewPayload(data) {
  ...
  const publicId = String(data?.cloudinary_public_id ?? "").trim();
  const resourceType =
